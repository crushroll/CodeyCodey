...
type Input
	...
	keyState as integer
	keyReleased as integer
	keyRepeat as integer
	keyRepeatTime as integer
	keyWaitTime as integer
	keyChr as string
	shiftDown as integer
	keyScan as integer
	keyAsc as integer
...
function inKeys()
	...
	if in.hasKeyboard
		...
		in.shiftDown = GetRawKeyState(KEY_SHIFT)
		in.keyScan = GetRawLastKey()
		in.keyAsc = inScanToAsc(in.keyScan, in.shiftDown)
		
		if in.keyScan <> KEY_SHIFT
			
			if GetRawKeyPressed(in.keyScan)
				...
				if in.keyRepeat
					...
					in.keyWaitTime = in.keyRepeat // KEY_WAIT_TIME
					
				endif
				
			elseif GetRawKeyState(in.keyScan)
				
				in.keyState = in.keyScan
				...
				if in.keyRepeat
					...
					if m - in.keyRepeatTime >= in.keyWaitTime
						...
						pressed = true
						in.keyRepeatTime = m
						in.keyWaitTime = in.keyRepeat // KEY_REPEAT_TIME
						
					endif
					
				endif
					
			elseif GetRawKeyReleased(in.keyScan)
				
				in.keyReleased = in.keyScan
				
			endif

			if pressed

				in.keyPressed = in.keyScan

				if in.keyPressed <> 0
					in.keyChr = chr(in.keyAsc)
				else
					in.keyChr = ""
				endif
